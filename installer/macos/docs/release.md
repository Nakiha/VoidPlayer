...
- `dev.py package` verifies the staged FFmpeg dylib layout with `otool -L`,
  signs the staged app with Release entitlements after adding bundled notices,
  then verifies it with `codesign --verify --deep --strict`. By default this
  uses ad-hoc signing for local testing. Pass `--macos-sign-identity`, or set
