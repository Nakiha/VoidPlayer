...
PresentDecision RenderSink::evaluate() {
    ...
    debug_eval_count_++;
    if (debug_eval_count_ <= 10 && !tracks_.empty()) {
        auto peek_frame = tracks_[0]->peek(0);
        if (peek_frame) {
            spdlog::debug("[RenderSink] eval #{}: clock={:.3f}s frame_pts={:.3f}s dur={:.1f}ms",
                         debug_eval_count_, current_pts_us / 1e6,
                         peek_frame->pts_us / 1e6,
                         peek_frame->duration_us / 1e3);
        } else {
            spdlog::debug("[RenderSink] eval #{}: clock={:.3f}s NO FRAME", debug_eval_count_, current_pts_us / 1e6);
        }
    }
