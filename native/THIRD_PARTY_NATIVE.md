...
| FFmpeg analyzer fork | submodule commit `a36106d4723f88e9c7d00e773767a46e1c432085` | `native/analysis/vendor/ffmpeg` | FFmpeg upstream licenses plus VoidPlayer analyzer patch licensing | `native/analysis/vendor/ffmpeg` | H.264/HEVC VBS4 analysis generation | Update submodule pointer, then rebuild analyzer via `dev.py`; keep stamp logic current. |
| VTM / NextSoftware | VTM `23.14`, submodule commit `d4715778f5151171616728fe9beecdc834b67e4e` | `native/analysis/vendor/vtm` | VTM upstream license | `native/analysis/vendor/vtm` | VVC/H.266 VBS4 generation | Update submodule pointer and rebuild `DecoderApp.exe` via `dev.py vtm build`. |
| zstd | 1.5.7, submodule commit `f8745da6ff1ad1e7bab384bd1f9d742439278e99` | `native/analysis/vendor/zstd` | BSD/GPL dual license, see `LICENSE` and `COPYING` | `native/analysis/vendor/zstd` | VBS4 compression/decompression static library | Update submodule pointer; CMake builds `libzstd_static` with tests/programs disabled. |
