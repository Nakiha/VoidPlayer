...
- [ ] Simplify main-window coordinator dependencies by grouping state and side effects behind explicit services/stores.
  Slices done: `MainWindowPlaybackCoordinator` now depends on `MainWindowStateStore`, the timeline hover notifier, `PlaybackPreferences`, and shared `MainWindowTimelineMetrics` for playback/seek/loop state instead of a long list of individual state getter/setter closures or config singleton reads; `MainWindowMediaCoordinator` now reads/writes viewport, layout, sync offset, duration, pending seek, and audible-track state through `MainWindowStateStore`, and uses `MainWindowTimelineMetrics` rather than owning effective-duration calculation for playback; `MainWindowLayoutCoordinator` now reads layout/texture state through `MainWindowStateStore` and track geometry through `TrackManager`; `MainWindowController` now delegates fullscreen/bounds side effects through an injected `MainWindowPlatform`.
