...
class ActionFocus extends StatefulWidget {
  final Widget child;
  // 内部: 向 HardwareKeyboard 注册全局 handler，并维护 ActionFocus 节点
}
```
...
- loop range 尾部拖动等不应触发 seek 的交互，可先 `STORE_NATIVE_SEEK_COUNT`，再用 `ASSERT_NATIVE_SEEK_COUNT_DELTA` 直接约束 native 插件 seek 次数。
