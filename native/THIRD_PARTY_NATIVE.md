...
| --- | --- | --- | --- | --- | --- | --- |
| FFmpeg runtime/dev package | FFmpeg `n8.1`, pinned by `toolchains/ffmpeg.lock.json` | `https://github.com/Nakiha/VoidPlayer-FFmpeg-Build` | GPL v3 package, see hydrated package license files | `.toolchains/ffmpeg/windows-x64` and `.toolchains/ffmpeg/macos-arm64` | demux, decode, D3D11VA/D3D12VA, VideoToolbox, hwdownload, swresample | Update `toolchains/ffmpeg.lock.json`, restore scripts, and package metadata together; do not commit hydrated SDK files. |
...
| Rust wgpu stack | `wgpu`/`wgpu-hal` 29.0.3, `objc2` 0.6.4, `objc2-metal` 0.3.2, `pollster` 0.4.0, pinned by `native/rust/Cargo.lock` | `https://github.com/gfx-rs/wgpu`, `https://github.com/madsmtm/objc2`, `https://github.com/zesterer/pollster` | `wgpu`/`wgpu-hal`: MIT OR Apache-2.0; `objc2`: MIT; `objc2-metal`: Zlib OR Apache-2.0 OR MIT; `pollster`: Apache-2.0 OR MIT | Cargo registry cache / `native/rust` build output | experimental macOS wgpu-metal presentation backend and Metal texture import bridge | Rust toolchain is pinned by `native/rust/rust-toolchain.toml`; keep `Cargo.lock`, feature flags, FFI ABI, and this manifest in sync. |
...
required FFmpeg DLLs from `.toolchains/ffmpeg/windows-x64/bin`. The staging rules copy the
...
aliases from `.toolchains/ffmpeg/macos-arm64/lib` into `.app/Contents/Frameworks`, then copy
