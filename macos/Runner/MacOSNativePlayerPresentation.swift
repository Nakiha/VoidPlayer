...
extension MacOSNativePlayerSession {
  ...
  func commitSourceProviderSeekPreview(timeoutMs: Int) throws -> MacOSNativeFrameInfo {
    var info = VPMacOSNativeFrameInfo()
    var error = [CChar](repeating: 0, count: 1024)
    let ret = VPMacOSNativePlayerCommitSourceProviderSeekPreview(
      handle,
      Int32(max(0, timeoutMs)),
      &info,
      &error,
    ...
    if ret != 0 {
      ...
      throw MacOSNativePlayerError.transientFrameUnavailable(
        message.isEmpty ? "source-provider seek preview is not ready" : message
      )
    }
    guard info.width > 0, info.height > 0 else {
      throw MacOSNativePlayerError.invalidPayload
