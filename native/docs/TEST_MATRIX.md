...
| `ui_tests/smoke/windows_dcomp_pacing_probe.csv` | SDR DComp retained graph pacing probe; verifies source/Flutter bake counts, projection-only commits, and zero final swap-chain present wait during playing pan/split/hover. | Targeted DComp/presentation performance |
| `ui_tests/smoke/windows_dcomp_pacing_probe_scrgb.csv` | Forced scRGB DComp retained graph pacing probe; verifies FP16 retained source/Flutter surfaces and zero final swap-chain present wait during playing pan/split/hover. | Targeted DComp/presentation performance |
