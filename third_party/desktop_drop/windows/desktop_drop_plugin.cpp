...
namespace {
    ...
    std::string global_unicode_text_to_utf8(HGLOBAL global) {
        auto data = static_cast<const wchar_t *>(GlobalLock(global));
        if (data == nullptr) {
            return {};
        }
        std::wstring wide(data);
        GlobalUnlock(global);
        return ws2s(wide);
    }

    std::string global_ansi_text_to_utf8(HGLOBAL global) {
    ...
    void append_text_format(IDataObject *data_object,
                            UINT format,
                            bool unicode,
                            std::vector<std::string> &values) {
        FORMATETC fmtetc = {static_cast<CLIPFORMAT>(format), nullptr, DVASPECT_CONTENT, -1, TYMED_HGLOBAL};
        STGMEDIUM stgmed;
        if (data_object->QueryGetData(&fmtetc) != S_OK) {
            return;
        }
        if (data_object->GetData(&fmtetc, &stgmed) != S_OK) {
            return;
        }
        std::string text = unicode
            ? global_unicode_text_to_utf8(stgmed.hGlobal)
            : global_ansi_text_to_utf8(stgmed.hGlobal);
    ...
    HRESULT DesktopDropTarget::Drop(IDataObject *pDataObj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect) {
        ...
        append_text_format(pDataObj, CF_UNICODETEXT, true, values);
        append_text_format(pDataObj, CF_TEXT, false, values);
        append_text_format(pDataObj, RegisterClipboardFormat(L"UniformResourceLocatorW"), true, values);
        append_text_format(pDataObj, RegisterClipboardFormat(L"UniformResourceLocator"), false, values);

        for (const auto &value : values) {
            list.push_back(flutter::EncodableValue(value));
        }
