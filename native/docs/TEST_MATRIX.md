...
| Windows preservation | Windows runner/wgpu-D3D12 presentation preservation after shared renderer/backend changes. | `python dev.py gate windows-preservation` |
...
| `ui_tests/smoke/native_seek_preview_event_dcomp_scrgb.csv` | Locked-engine Flutter export, D3D12 external Flutter surface update/consume, DComp activation/ACK, seek, resize, viewport alpha, and fail-closed compositor failure. | Windows preservation / targeted presentation changes |
| `ui_tests/smoke/native_compositor_flutter_surface_passive_scrgb.csv` | Locked-engine compositor-owned Flutter Surface passive for settings, media-info, toolbar hover, export publish/request diagnostics, and stale-generation fail-closed coverage. | Windows preservation / targeted presentation changes |
| `ui_tests/smoke/windows_flutter_surface_churn_probe.csv` | Locked-engine Flutter UI churn probe; real controlbar play toggles plus toolbar/panel toggles must produce enough Flutter frames and passive Flutter surface publishes without stale/backpressure failures. | Windows Flutter surface / toolbar animation changes |
| `ui_tests/smoke/settings_storage_tabs_pacing_probe.csv` | Locked-engine DComp probe for Flutter-only Settings animations; storage tab switches must maintain Flutter timing and fresh passive Flutter surface publishes without stale/backpressure failures. | Windows Flutter export / settings animation changes |
| `ui_tests/smoke/native_compositor_window_close_scrgb.csv` | Windows native compositor close path through `WindowManager.close`, ensuring the main window hides before native/DComp teardown so no native-surface teardown flash is shown. | Windows preservation / targeted presentation changes |
| `ui_tests/smoke/native_source_projection_dcomp_scrgb.csv` | Paused/playing source cache, wgpu/D3D12 projection consume, split, analysis overlay projection, seek/resize, diagnostics, and fail-closed cleanup. | Windows preservation / targeted presentation changes |
...
| `ui_tests/smoke/native_high_refresh_overlay_pan_zoom.csv` | Forced scRGB source-projection overlay interaction; verifies wgpu overlay/source reuse, no viewport redraw during projection, retained D3D11 graph inactive, and visible output. | Windows preservation / `windows-high-refresh-local` |
| `ui_tests/smoke/windows_dcomp_pacing_probe.csv` | SDR pacing probe for playing pan/split/hover; verifies wgpu/D3D12 source projection, external Flutter surface consume, retained graph inactive/zero, and present cadence. | Targeted DComp/presentation performance |
| `ui_tests/smoke/windows_dcomp_pacing_probe_scrgb.csv` | Forced scRGB pacing probe for playing pan/split/hover; verifies wgpu/D3D12 source projection, external Flutter surface consume, retained graph inactive/zero, and present cadence. | Targeted DComp/presentation performance |
| `ui_tests/smoke/windows_hdr_auto_runtime.csv` | Adds/removes an HLG track on an HDR display and verifies Auto records desired scRGB while falling back to native SDR with `hdr-ui-composition-unsupported` until SDR Flutter UI can remain system-managed. | Local `windows-hdr-auto`; requires Windows HDR enabled |
