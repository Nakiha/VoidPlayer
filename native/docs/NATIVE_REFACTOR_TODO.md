...
- [x] 将 FFI diagnostics active-player lookup 从 process-global player registry 收口到 host/session scope。
...
- [x] 给 DecodeThread codec send/receive、AVFrame ownership 和硬解 visibility flush 增加更小粒度边界测试。
...
- `video_renderer_lib` 仍固定链接 D3D11/DXGI/d3dcompiler/winmm，后续 target 边界还未拆细。
- `BUILD_ANALYSIS=OFF` 已能让 renderer/player/FFI 不链接 `analysis_lib`，analysis overlay draw path 降级为 no-op stub。
