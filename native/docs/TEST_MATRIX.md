...
| `ui_tests/macos/native_remove_hlg_paused_pan_smoke.csv` | Removes an HLG track from a mixed SDR+HLG session while playing, then verifies the remaining SDR complete target still responds to paused pan. | Targeted HDR policy / target-ring / track removal regressions. |
...
| `ui_tests/macos/analysis_gated_smoke.csv` | macOS analysis FFI, inline analysis page, and media-header overlay panel/activation. | Nightly/headed or targeted analysis changes. |
