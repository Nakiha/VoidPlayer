...
| `windows_d3d11_dcomp_flutter_composite_smoke` | backend contract | PR fast / Windows preservation | D3D11 hardware adapter; WARP accepted for hosted contract coverage | Legacy compatibility canary for D3D11 compositor shader behavior. Product Flutter UI composition evidence must come from the wgpu/D3D12 external Flutter surface probes. |
