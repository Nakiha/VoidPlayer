...
| `ui_tests/smoke/native_source_projection_device_recovery.csv` | Debug-injected source-cache device-loss recovery while source projection is active; verifies cache republish, pan/zoom, split, seek, and EventChannel health. | Windows preservation / targeted device recovery changes |
