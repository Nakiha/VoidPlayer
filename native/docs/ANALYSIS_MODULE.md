...
├── parsers/                    # 二进制文件解析器（只读）
│   ├── binary_types.h          # VBS3/VBI/VBT packed 结构体定义
...
- [VBS3](formats/VBS3.md) — `.vbs3`，VTM block statistics / CU 统计，当前 magic `VBS3`
...
VBS3 生成由 VTM DecoderApp 外部进程完成（可选），通过 `analysis_ffi.cpp` 调用。
...
| FFI 函数 | 功能 |
|----------|------|
| `naki_analysis_generate` | 生成 VBI+VBT（C++ FFmpeg），可选 VBS3（VTM DecoderApp） |
