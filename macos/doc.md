...
`addTrack`, and feeds the shared present decision into the Metal layout uploader for same-sized
BGRA multi-track composition. The same facade now compiles and owns the shared native audio engine on
...
`CVPixelBuffer`. For same-sized multi-track BGRA decisions, native stages a per-track atlas from
`RenderSink::PresentDecision` and the Metal compute pass uses the shared layout constants generated
by `populate_layout_shader_constants`, matching the DX11 layout contract instead of adding Swift
composition policy. The older
