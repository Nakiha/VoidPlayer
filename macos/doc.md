...
unsupported non-BGRA surfaces. Explicit seek/step refresh and playback frame callbacks now report
the actual native upload storage: `metal-cvpixelbuffer-present-package` for renderer-owned
VideoToolbox frames, `metal-yuv-present-package` for staged NV12/P010/planar software frames, and
`metal-bgra-present-package` for BGRA fallback packages. Package uploads copy into a shared
`MTLBuffer` when staging is required, then a Metal compute pass applies the shared layout transform
into the texture-backed `CVPixelBuffer`. For multi-track BGRA decisions, native stages a per-track atlas from
`RenderSink::PresentDecision`, preserves each track's source dimensions, and the Metal compute pass
uses the shared layout constants generated by `populate_layout_shader_constants`, matching the DX11
layout contract instead of adding Swift composition policy. Single-track VideoToolbox
renderer-owned frames can now be presented by wrapping the retained decoder `CVPixelBuffer` planes
with `CVMetalTextureCache`, avoiding the hwdownload/package staging bottleneck. Multi-track
