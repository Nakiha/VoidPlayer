...
bool AudioDecodeThread::start() {
    ...
    if (avcodec_parameters_to_context(codec_ctx_.get(), codec_params_) < 0) {
        spdlog::warn("[AudioDecodeThread] avcodec_parameters_to_context failed");
        codec_ctx_.reset();
        return false;
    }
    if (codec_ctx_.get()->ch_layout.nb_channels <= 0) {
        av_channel_layout_default(&codec_ctx_.get()->ch_layout, codec_params_->ch_layout.nb_channels > 0
...
bool AudioDecodeThread::init_resampler() {
    ...
    int ret = swr_alloc_set_opts2(
        ...
        &codec_ctx_.get()->ch_layout,
        codec_ctx_.get()->sample_fmt,
        codec_ctx_.get()->sample_rate,
        0,
        nullptr);
...
void AudioDecodeThread::flush_after_seek_if_needed() {
    ...
    if (swr_) swr_close(swr_.get());
    if (swr_) swr_init(swr_.get());
...
void AudioDecodeThread::receive_frames(AVFrame* frame) {
    while (running_.load()) {
        ...
        const int out_capacity = static_cast<int>(
            av_rescale_rnd(
                swr_get_delay(swr_.get(), codec_ctx_.get()->sample_rate) + frame->nb_samples,
                kAudioOutputSampleRate,
                codec_ctx_.get()->sample_rate,
                AV_ROUND_UP));
